...
*
* @package		Part of Speech
* @subpackage	Part of Speech Library
* @author		Brennan Novak (converted to CodeIgniter), originally cased on the Brill tagger coded by Mark Watson
* @link			http://phpir.com/part-of-speech-tagging | http://www.markwatson.com/opensource/
*
*/
...
class Post_tagger
{
    ...
    {
        $fh = fopen(APPPATH.'modules/emoome/language/english/lexicon.txt', 'r');
    ...
    public function tag($text)
    {
        ...
        foreach($matches[0] as $token)
        {
            ...
            if ($i > 0)
            {
	            if($return[$i - 1]['tag'] == 'DT' && in_array($return[$i]['tag'], array('VBD', 'VBP', 'VB')))
	            {
                    $return[$i]['tag'] = 'NN';
	            }
            }
            ...
            if(in_array($return[$i]['tag'], $nouns) && substr($token, -2) == 'al')
            {
				$return[$i]['tag'] = 'JJ';
            }
            ...
            if($i > 0)
            {
				if($return[$i]['tag'] == 'NN' && strtolower($return[$i-1]['token']) == 'would')
				{
				    $return[$i]['tag'] = 'VB';
				}
            }
            ...
            if($i > 0)
            {
                if(in_array($return[$i]['tag'], $nouns) && in_array($return[$i-1]['tag'], $nouns) && isset($this->dict[strtolower($token)]))
                {
